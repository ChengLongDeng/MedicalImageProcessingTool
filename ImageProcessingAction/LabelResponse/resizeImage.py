...
from FileAction.readImageByCV import opencv2pil, pil2opencv, imageConvertion
...
def enlargeImage(var, lvar, width, height):
    ...
    if var.width < 800:
        ...
        var.trImageShow.setPixmap(QPixmap(imageConvertion(var)))        # 从新显示在Label中
...
def restoreImageToDefault(var, lvar):
    ...
    var.img = opencv2pil(var.img)       # opencv打开图像转换成PIL形式
    var.img = resizeimage.resize_cover(var.img, [512, 512], validate=False)
    # var.img.save(var.im_path)
    var.img = pil2opencv(var.img)       # PIL类型图像转换成opencv形式
    var.size.setText('(%d × %d)' % (var.width, var.height))  # 状态栏显示图像大小